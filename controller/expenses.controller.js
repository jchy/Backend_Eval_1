...
const groupExpenseByTypeAndSort =  async (req,res)=>{
    try{
        const per_page = req.query.per_page || 2;
        const page = req.query.page || 1;
        const skip = page < 0 ? 0 : (page - 1)*per_page;
...
const getAvgeExpenseReimburse =  async (req,res)=>{
    try{
        const per_page = req.query.per_page || 2;
        const page = req.query.page || 1;
        const skip = page < 0 ? 0 : (page - 1)*per_page;
...
const getExpenses_Bw_Date1_and_Date2_sortby_asc =  async (req,res)=>{
    try{
        const per_page = req.query.per_page || 2;
        const page = req.query.page || 1;
        const skip = page < 0 ? 0 : (page - 1)*per_page;
...
const removeExpenseByEmployeeId = async (req,res)=>{
    try{
